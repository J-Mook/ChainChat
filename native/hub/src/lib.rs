...
async fn main() {
    ...
    let forward_ip_addr = SocketAddr::new(Ipv4Addr::new(127, 0, 0, 1).into(), 8002);
    let backward_ip_addr = SocketAddr::new(Ipv4Addr::new(127, 0, 0, 1).into(), 8004);

    let self_addr = "127.0.0.1:8003";
    let mut buf = [0; 1024];

    tokio::spawn(async move {
        let socket = UdpSocket::bind(self_addr).await.unwrap();
        loop {
